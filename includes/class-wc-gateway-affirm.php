...
class WC_Gateway_Affirm extends WC_Payment_Gateway {
	...
	public function handle_wc_api() {
		...
		try {
			...
			require_once( 'class-wc-gateway-affirm-charge-api.php' );
			$charge_api = new WC_Gateway_Affirm_Charge_API( $this, $order_id );

			$result = $charge_api->request_charge_id_for_token( $checkout_token );
			...
			if ( ! $validates ) {
				$charge_api->void_charge( $charge_id );
				throw new Exception(
			...
			$this->update_order_meta( $order_id, 'charge_id', $charge_id );
			...
			if ( $this->auth_only_mode ) {
				...
				$this->set_order_auth_only_flag( $order );
	...
	public function capture_charge( $order ) {
		...
		$this->clear_order_auth_only_flag( $order );
		...
		$charge_id = $this->get_order_meta( $order_id, 'charge_id' );

		if ( ! $charge_api->capture_charge( $charge_id ) ) {
	...
	public function void_charge( $order ) {
		...
		if ( ! $charge_api->void_charge( $charge_id ) ) {
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'public_key' => array(
				...
				'description' => sprintf(
					/* translators: 1: html starting code 2: html end code */
					__( 'This is the public key assigned by Affirm and available from your %1$smerchant dashboard%2$s (%3$sSandbox%4$s).', 'woocommerce-gateway-affirm' ),
			...
			'private_key' => array(
				...
				'description' => sprintf(
					/* translators: 1: html starting code 2: html end code */
					__( 'This is the private key assigned by Affirm and available from your %1$smerchant dashboard%2$s (%3$sSandbox%4$s).', 'woocommerce-gateway-affirm' ),
					'<a target="_blank" href="https://www.affirm.com/dashboard/" class="woocommerce_affirm_merchant_dashboard_link">',
					'</a>',
					'<a target="_blank" href="https://sandbox.affirm.com/dashboard/" class="woocommerce_affirm_merchant_dashboard_link">',
					'</a>'
	 ...
	 * @since 1.0.0
	 */
	function is_valid_for_administration() {
	...
	function admin_options() {
		if ( $this->is_valid_for_administration() ) {
	...
	function is_valid_for_use() {
		if ( $this->is_current_page_requires_ssl() && ! is_ssl() ) {
			return false;
		}
	 ...
	 */
	public function is_current_page_requires_ssl() {
		if ( $this->testmode ) {
			return false;
		}
	...
	public function can_refund_order( $order ) {
		return ( $order && ( $this->isset_order_auth_only_flag( $order ) || $order->get_transaction_id() ) );
	}
	...
	public function process_refund( $order_id, $refund_amount = null, $reason = '' ) {
		...
		if ( $this->isset_order_auth_only_flag( $order ) ) {
			...
			$result = $charge_api->void_charge( $charge_id );
		...
		$charge_id = $order->get_transaction_id();
		$result = $charge_api->refund_charge( $charge_id, $refund_amount_in_cents );
	...
	function review_order_before_payment() {
		...
		if ( ! $this->is_checkout_auto_post_page() ) {
			return;
		}

		$order = $this->validate_order_from_request();
		if ( false === $order ) {
	 ...
	 * @since 1.0.0
	 */
	function is_checkout_auto_post_page() {
		if ( ! is_checkout() ) {
			return false;
		}

		if ( ! isset( $_GET['affirm'] ) || ! isset( $_GET['order'] ) || ! isset( $_GET['nonce'] ) ) {
			return false;
		}
	 ...
	 * @since 1.0.0
	 */
	function validate_order_from_request() {
		if ( empty( $_GET['order'] ) ) {
			return false;
		}
	...
	function enqueue_scripts() {
		...
		$affirm_data = array(
			...
			'items'     => $this->get_items_formatted_for_affirm( $order ),
			'discounts' => array(
				'discount' => array(
	 ...
	 * @since 1.0.0
	 */
	function get_items_formatted_for_affirm( $order ) {
		...
		foreach ( (array) $order->get_items( array( 'line_item', 'fee' ) ) as $item ) {
			...
			$sku = '';
			$unit_price = 0;
			$qty = array_key_exists( 'qty', $item ) ? intval( $item['qty'] ) : 1;
	...
	function admin_enqueue_scripts( $hook ) {
		...
		if ( ! isset( $_GET['section'] ) || 'wc_gateway_affirm' !== $_GET['section'] ) {
			return;
		}
	...
	/**
	 * Helper methods to manage order auth flags
	 */
	public function isset_order_auth_only_flag( $order ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		return $this->get_order_meta( $order_id, 'authorized_only' );
	}

	public function set_order_auth_only_flag( $order ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		return $this->update_order_meta( $order_id, 'authorized_only', true );
	}

	public function clear_order_auth_only_flag( $order ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		return $this->delete_order_meta( $order_id, 'authorized_only' );
	}
